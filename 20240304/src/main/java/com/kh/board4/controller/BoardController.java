...
public class BoardController {
	...
	public String index(Model model, @RequestParam(defaultValue = "0") int page) {
		...
		// PageRequst.of()메서드를 이용해서 PageRequest
		// 객체를 생성할 수 있다.
	...
	public String reply(@RequestParam Long boardNo, @RequestParam String writerId, @RequestParam String content,
	...
	public String editBoard(@PathVariable Long no, Model model) {
		// no에 해당하는 게시물을 조회하여 모델에 추가하고, 수정 페이지로 이동
		Board board = boardRepository.findById(no).orElse(null);
		model.addAttribute("board", board);
		return "board/update"; // 수정 페이지의 뷰 이름
	}

	@PostMapping("/board/edit/{no}")
	public String processEditBoard(@PathVariable Long no,
			@ModelAttribute Board updatedBoard) {
		// no에 해당하는 게시물을 업데이트하고, 상세 페이지로 이동
		Board existingBoard = boardRepository.findById(no).orElse(null);
		...
		return "redirect:/board/detail/{no}"; // 상세 페이지로 이동
	}
